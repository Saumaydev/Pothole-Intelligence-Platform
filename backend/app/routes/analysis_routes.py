...
async def get_result(
    ...
    if db_analysis.status != "completed":
        raise HTTPException(
            400,
            f"Analysis not yet complete. Status: {db_analysis.status}"
        )
    ...
    return AnalysisResponse(
        ...
        severity_distribution=meta.get(
            "severity_distribution",
            {}
        ),
        confidence_scores=meta.get(
            "confidence_scores",
            []
        ),
        density_along_road=meta.get(
            "density_along_road",
            []
        ),
        cumulative_potholes=meta.get(
            "cumulative_potholes",
            []
        ),
...
async def run_full_analysis(
    ...
    try:
        ...
        db_analysis.city = geo_data.get(
            "city",
            request.city
        )
        db_analysis.state = geo_data.get(
            "state",
            request.state
        )
        ...
        _update_progress(
            ...
            "road_extraction",
            15,
            "Extracting road geometry from OpenStreetMap..."
        )
        ...
        db_analysis.road_length_km = road_data[
            "total_length_km"
        ]

        db_analysis.road_type = road_data.get(
            "road_type",
            request.road_type or "urban"
        )

        db_analysis.road_polyline = [
            [lat, lng]
            for lat, lng in road_data["coordinates"]
        ]
        ...
        _update_progress(
            ...
            "sampling",
            25,
            "Sampling points along road..."
        )

        sampling_dist = (
            request.sampling_distance
            or settings.SAMPLING_DISTANCE_METERS
        )

        sampled_points = sample_points_along_line(
            road_data["coordinates"],
            sampling_dist
        )

        db_analysis.total_points_sampled = len(
            sampled_points
        )
        ...
        _update_progress(
            ...
            "image_acquisition",
            35,
            f"Acquiring {len(sampled_points)} street-level images..."
        )

        image_results = await image_service.acquire_images(
            sampled_points
        )
        ...
        _update_progress(
            ...
            "detection",
            55,
            "Running YOLOv8 pothole detection..."
        )

        detection_results = (
            detection_service.detect_potholes_batch(
                image_results
            )
        )
        ...
        _update_progress(
            ...
            "analysis",
            75,
            "Aggregating results and computing statistics..."
        )

        agg_results = (
            analysis_service.aggregate_results(
                detection_results,
                sampled_points,
                road_data
            )
        ...
        _update_progress(
            ...
            "speed_prediction",
            85,
            "Predicting speed impact..."
        )

        speed_results = (
            speed_service.predict_speed_impact(
                agg_results["segments"],
                db_analysis.road_type
            )
        )
        ...
        speed_map = {
            s["segment_index"]: s
            for s in speed_results["segments_speed"]
        }
        ...
        for seg in agg_results["segments"]:
            ...
            seg["predicted_speed"] = sp.get(
                "predicted_speed",
                speed_results["base_speed"]
            )

            seg["speed_reduction"] = sp.get(
                "speed_reduction_pct",
                0
            )
        ...
        db_analysis.total_potholes_detected = (
            agg_results["total_potholes"]
        )

        db_analysis.avg_pothole_density = (
            agg_results["avg_density"]
        )

        db_analysis.max_pothole_density = (
            agg_results["max_density"]
        )

        db_analysis.overall_risk_level = (
            agg_results["overall_risk"]
        )

        db_analysis.avg_predicted_speed = (
            speed_results["avg_predicted_speed"]
        )

        db_analysis.min_predicted_speed = (
            speed_results["min_predicted_speed"]
        )

        db_analysis.speed_reduction_pct = (
            speed_results["speed_reduction_pct"]
        )

        db_analysis.base_speed = (
            speed_results["base_speed"]
        )

        db_analysis.heatmap_data = (
            agg_results["heatmap_data"]
        )

        db_analysis.analysis_metadata = {
            "speed_analysis": speed_results,
            "severity_distribution": (
                agg_results["severity_distribution"]
            ),
            "confidence_scores": (
                agg_results["confidence_scores"]
            ),
            "density_along_road": (
                agg_results["density_along_road"]
            ),
            "cumulative_potholes": (
                agg_results["cumulative_potholes"]
            ),
        }
        ...
        for seg in agg_results["segments"]:
            db_seg = RoadSegment(
                ...
                predicted_speed=seg.get(
                    "predicted_speed"
                ),
                speed_reduction=seg.get(
                    "speed_reduction"
                ),
            )
